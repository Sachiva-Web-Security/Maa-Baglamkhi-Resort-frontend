const MetricCard = ({ title, value, subtitle, icon: Icon, gradient, onClick }) => {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="
        dashboard-card group relative h-full min-h-[128px] cursor-pointer overflow-hidden
        px-5 py-4 text-left transition-all duration-300 hover:-translate-y-0.5
    ...
    >
      ...
      <div className="relative grid h-full grid-cols-[minmax(0,1fr)_auto] items-start gap-3 sm:gap-3.5">
        <div className="min-w-0">
          <div className="dashboard-label inline-flex w-fit max-w-full items-center gap-1.5 rounded-full bg-slate-50 px-2.5 py-1">
            <span className={`block h-2 w-2 rounded-full ${gradient}`} />
        ...
        <div
          className={`flex h-11 w-11 shrink-0 items-center justify-center self-start rounded-xl text-white shadow-[0_10px_24px_rgba(15,23,42,0.12)] transition duration-300 group-hover:scale-105 sm:h-12 sm:w-12 xl:h-12 xl:w-12 ${gradient}`}
        >
          {iconEl}
        </div>
      </div>
