...
const BanquetWhatsAppModal = ({ booking, onClose, onSuccess }) => {
  const initialPhone = stripCountryCode(booking?.phone || "");
  ...
  useEffect(() => {
    if (!booking) return;
    const parsed = stripCountryCode(booking.phone || "");
  ...
  const handleSend = async () => {
    if (!booking?.invoiceNo) {
      ...
      setStatusMessage("Invoice not generated yet. Please generate the bill first.");
      return;
    }
    ...
    try {
      ...
      const response = await API.post(
        `/banquet/invoice/send-whatsapp/${booking.id}`,
        {
      ...
      if (error.response?.status === 404) {
        setStatus(STATUS.UNAVAILABLE);
        setStatusMessage("Booking not found. It may have been deleted.");
      } else if (error.response?.status === 500 && /wasachiva|key not configured/i.test(message)) {
        setStatus(STATUS.UNAVAILABLE);
        ...
        setStatus(STATUS.UNAVAILABLE);
        setStatusMessage("Request timed out. The server may be slow or unreachable. Please try again.");
      } else {
        setStatus(STATUS.FAILED);
        setStatusMessage(message);
      }
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[12px] opacity-90 sm:text-[13px]">
                Banquet · Invoice #{booking?.invoiceNo || "--"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="space-y-4 p-4 sm:p-5">
          {!booking?.invoiceNo ? (
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-3 text-[14px] font-semibold text-amber-800">
              Invoice not generated yet. Please generate the bill first.
            </div>
          ) : null}
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            ...
            <button
              type="button"
              onClick={handleSend}
              disabled={isSending || Boolean(phoneError) || !customerName.trim() || !phoneLocal}
...
export default BanquetWhatsAppModal;
export { STATUS };
