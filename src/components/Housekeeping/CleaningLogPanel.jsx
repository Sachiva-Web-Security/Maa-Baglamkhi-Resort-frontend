...
export default function CleaningLogPanel({
  ...
  const handleExtendClick = (room) => {
    if (!onExtendCleaningTime) return;
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="mt-1 text-sm text-slate-600">
              Aaj total <span className="font-bold text-slate-900">{completedTodayRows.length}</span>{" "}
              room clean mark hue hain.
            </div>
          </div>
        ...
        <>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {paginatedRows.map((room) => (
              <div key={`clean-${room.id}`} className="relative overflow-visible">
                ...
                <div className="overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-[0_18px_38px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_24px_50px_rgba(15,23,42,0.12)]">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4 space-y-2 text-sm text-slate-600">
                      <p>
                        Assignee:{" "}
                        <span className="font-semibold text-slate-900">{room.assignee}</span>
                      </p>
                      <p>
                        Guest:{" "}
                        <span className="font-semibold text-slate-900">{room.guestStatus || "-"}</span>
                      </p>
                    </div>

                    <div
                      className={`mt-4 rounded-[1.2rem] border px-3 py-3 ${
                        room.isOverdue ? "border-rose-200 bg-rose-50" : "border-violet-200 bg-violet-50"
                      }`}
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="mt-1 text-[11px] font-medium text-slate-500">
                            Start {formatTimeRange(room.task?.startedAt)} | Due{" "}
                            {formatTimeRange(room.task?.dueAt)}
                          </div>
                        </div>
                      </div>
