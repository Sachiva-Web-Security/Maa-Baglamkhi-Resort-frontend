...
const IDCards = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to load ID types");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSave = async () => {
    ...
    try {
      if (modal.mode === "add") {
        await API.post("/id-types", { name });
        setMessage("ID type added.");
      } else {
        await API.put(`/id-types/${modal.id}`, { name });
        setMessage("ID type updated.");
      }
      closeModal();
      await load();
    } catch (err) {
  ...
  const onDelete = async (row) => {
    ...
    setMessage("");
    try {
      await API.delete(`/id-types/${row.id}`);
      setMessage("ID type deleted.");
      await load();
    } catch (err) {
  ...
  return (
    <div style={styles.page}>
      <div style={styles.topbar}>
        <h2 style={styles.title}>Manage ID Type</h2>
      ...
      <div style={styles.subtitle}>List of ID Types</div>
        ...
        <table style={styles.table}>
          <thead>
            <tr>
              ...
              <th style={styles.th}>ID Types</th>
            ...
            {rows.length === 0 && !loading && (
              <tr>
                <td colSpan={3} style={styles.empty}>
                  No ID types yet. Click <b>+</b> to add one.
                </td>
              </tr>
            )}
            {rows.map((row, idx) => (
        ...
        <div style={styles.modalBackdrop} onClick={closeModal}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              {modal.mode === "add" ? "Add ID Type" : "Edit ID Type"}
              <button onClick={closeModal} style={styles.modalClose}>
                ×
              </button>
            </div>
            <div style={styles.modalBody}>
              <label style={styles.label}>ID Type Name</label>
              <input
                style={styles.input}
                autoFocus
                value={modal.name}
