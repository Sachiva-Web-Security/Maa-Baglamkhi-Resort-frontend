...
const TaxCategory = () => {
  ...
  const load = async () => {
    ...
    setError("");
    try {
      const { data } = await API.get("/tax-categories");
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      setError(err.response?.data?.message || "Failed to load tax categories");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSave = async () => {
    ...
    try {
      if (modal.mode === "add") {
        await API.post("/tax-categories", { name });
        setMessage("Category added.");
      } else {
        await API.put(`/tax-categories/${modal.id}`, { name });
  ...
  const onDelete = async (row) => {
    ...
    setMessage("");
    try {
      await API.delete(`/tax-categories/${row.id}`);
  ...
  return (
    <div style={styles.page}>
      <div style={styles.topbar}>
        <h2 style={styles.title}>Manage Tax Category</h2>
            ...
            {rows.length === 0 && !loading && (
              <tr>
                <td colSpan={3} style={styles.empty}>
                  No tax categories yet. Click <b>+ New Category</b> to add one.
                </td>
              </tr>
            )}
            {rows.map((row, idx) => (
        ...
        <div style={styles.modalBackdrop} onClick={closeModal}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              {modal.mode === "add" ? "Add Tax Category" : "Edit Tax Category"}
