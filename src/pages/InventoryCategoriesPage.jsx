...
const ConfirmDelete = ({ open, onClose, onConfirm, name }) => (
  <Modal open={open} onClose={onClose} title="Delete category?" subtitle={`"${name || "this category"}" will be removed.`}
    ...
    <p className="text-[13.5px]">Are you sure? Menu items in this category will be kept but may appear uncategorized.</p>
  </Modal>
);
...
const CategoryFormModal = ({ open, onClose, onSave, initial }) => {
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial ? "Edit category" : "Add category"} subtitle={initial ? "Update category details" : "Create a new menu category"}
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="field"><label className={labelCls}>Category name *</label><input className={fieldCls} placeholder="e.g. Starters" value={form.name} onChange={(e) => setField("name", e.target.value)} /></div>
      </div>
    </Modal>
  );
};
...
const InventoryCategoriesPage = () => {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden" style={{ background: c.paper, color: c.text, padding: "22px 22px 60px" }}>
      <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-[21px] font-semibold m-0 leading-tight">Menu Categories</h1><p className="m-0 text-[12px]" style={{ color: c.muted }}>Organize your menu items into categories</p></div>
          </div>
        </div>
        <button type="button" onClick={() => { setEditingCat(null); setShowForm(true); }} className={primaryBtn}><FaPlus className="text-sm" /> Add Category</button>
      </div>
      <div className="mb-4"><div className="relative max-w-md"><FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-[#6B6F66] text-xs" /><input className={`${fieldCls} pl-9`} placeholder="Search categories…" value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
      <div className="bg-white border border-[#E4E1D8] rounded-[14px] overflow-hidden shadow-sm">
        <div className="overflow-x-auto"><table className="w-full min-w-[500px] text-left text-[13px]">
          ...
          <tbody className="divide-y divide-[#E4E1D8]">
            {loading ? <tr><td colSpan={2} className="py-10 text-center text-[#6B6F66]">Loading…</td></tr> : filtered.length === 0 ? (
              <tr><td colSpan={2}><div className="text-center py-9 text-[#6B6F66]"><b className="block text-[14.5px] text-[#1C231F] mb-1">No categories</b><span className="text-[13px]">Add your first category.</span></div></td></tr>
            ) : filtered.map((cat) => (
              <tr key={cat.id} className="hover:bg-[#F6F5F1]/50 transition-colors">
                <td className="py-2.5 pr-3 font-semibold text-[#1C231F]">{cat.name || "-"}</td>
                <td className="py-2.5"><div className="flex gap-1.5">
                  <button onClick={() => { setEditingCat(cat); setShowForm(true); }} className="border border-[#E4E1D8] bg-[#E4F0EE] text-[#0B4F48] hover:bg-[#CFE7E2] px-2.5 py-1.5 rounded-[9px] text-[12px] font-semibold transition-all"><FaEdit /></button>
                  <button onClick={() => setDeletingId(cat.id)} className={dangerBtn}><FaTrash /></button>
                </div></td>
              </tr>
            ))}
          </tbody>
      ...
      <CategoryFormModal open={showForm} onClose={() => { setShowForm(false); setEditingCat(null); }} onSave={handleSave} initial={editingCat} />
      <ConfirmDelete open={!!deletingId} onClose={() => setDeletingId(null)} onConfirm={handleDelete} name={categories.find((c) => c.id === deletingId)?.name} />
